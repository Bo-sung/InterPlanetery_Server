...
class GameState {
    ...
    class Player {
        ...
        ProductionQueueInfo[] productionQueue; // ⭐ 생산 대기열
        ...
        class ProductionQueueInfo {
            long fleetId;          // 생산 중인 함대 ID
            int fleetType;         // 함대 타입 (production_info.target_id)
            int ownerId;           // 소유자 ID (UI 구분용)
            int remainingTicks;    // 남은 생산 시간 (틱)
            int totalTicks;        // 총 생산 시간 (틱)
            float progress;        // 진행도 (0.0 ~ 1.0)
        }
    }
...
async Task HandleGameState(Protocol protocol) {
    ...
    foreach (var player in state.players) {
        ...
        // ⭐ 생산 대기열 업데이트
        UpdateProductionQueue(player.id, player.productionQueue);
    }
...
void UpdateProductionQueue(int playerId, ProductionQueueInfo[] queue) {
    // 생산 대기열 UI 업데이트
    foreach (var production in queue) {
        // 진행도 표시
        float progressPercent = production.progress * 100f;
        float remainingSeconds = production.remainingTicks / 20f; // 20 TPS

        ShowProductionUI(playerId, production.fleetType,
                        progressPercent, remainingSeconds);
    }

    // 대기열이 비어있으면 UI 숨김
    if (queue.Length == 0) {
        HideProductionUI(playerId);
    }
}
